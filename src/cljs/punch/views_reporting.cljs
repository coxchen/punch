...
(defn reporting-entries [group-by-proj proj include-log?]
  (fn []
    (into [:ul]
          (for [[idx entry] (map-indexed (fn [idx itm] [idx itm]) (get group-by-proj proj))]
            ...
            [:li
             ...
             (:topic entry)
             (if include-log? [reporting-logs entry])]))))

(defn reporting-projects [group-by-ver ver include-log?]
...
(defn reporting []
  (let [entries  (re-frame/subscribe [:entries])
    ...
    [:div
      [reporting-items entries versions "Progress" true]
      [reporting-items backlog versions "Plan" false]
      [reporting-problem]]))
